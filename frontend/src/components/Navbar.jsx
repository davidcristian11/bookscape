...
export default function Navbar() {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // Clear the local session even if the server-side session is already gone.
    } finally {
      clearAuthSession();
  ...
  const isAdmin = Boolean(user?.is_admin || user?.role === "admin" || user?.roles?.includes("admin"));
        ...
        {authenticated ? (
          <>
            <span className="nav-user">
              {user ? `Hi, ${user.name}${user.role ? ` (${user.role})` : ""}` : "Logged in"}
            </span>
            <motion.button
              type="button"
